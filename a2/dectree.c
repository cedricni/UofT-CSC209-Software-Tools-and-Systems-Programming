...
Dataset *load_dataset(const char *filename) {
    ...
    fread(&(data->num_items), sizeof(int), 1, file);
    data->images = malloc(sizeof(Image) * data->num_items);
    data->labels = malloc(sizeof(unsigned char) * data->num_items);
    int cnt = 0;
    while(1) {
        int temp = fread(&((data->labels)[cnt]), sizeof(unsigned char), 1, file);
        if (temp <= 0) {
            break;
        }
        data->images[cnt].data = malloc(sizeof(unsigned char) * NUM_PIXELS);
        fread(data->images[cnt].data, NUM_PIXELS, 1, file);
        data->images[cnt].sx = WIDTH;
        data->images[cnt].sy = WIDTH;
        cnt += 1;
    }
    return data;
}
...
int find_best_split(Dataset *data, int M, int *indices) {
    ...
    for (i = 0; i < NUM_PIXELS; i++) {
        imp = gini_impurity(data, M, indices, i);
        if (imp == NAN) {
            continue;
        }
        if (imp < cnt) {
...
int dec_tree_classify(DTNode *root, Image *img) {
    ...
    DTNode *curr = root;
    while (curr->classification < 0 || curr->classification > 9) {
        if (img->data[curr->pixel] == 255) {
            curr = curr->right;
        }
        else {
            curr = curr->left;
