...
int main(int argc, char *argv[]) {
    ...
    Dataset *training_data = load_dataset(argv[1]);
  	...
  	DTNode *tree = build_dec_tree(training_data);
  	int i;
  	int class;
  	for (i = 0; i < testing_data->num_items; i++) {
  		class = (int)(testing_data->labels[i]) - 48;
  		if (dec_tree_classify(tree, &((testing_data->images)[i])) == 48) {
  			total_correct += 1;
  		}
  	}
  	free_dec_tree(tree);
