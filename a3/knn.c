...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    int N;
    int correct = 0;
    if (read(p_in, &start_idx, sizeof(start_idx)) < 0) {
        perror("read");
        exit(1);
    };
    if (read(p_in, &N, sizeof(N)) < 0) {
        perror("read");
        exit(1);
    };
    int i;
    for (i = start_idx; i < start_idx + N; i++) {
        if (knn_predict(training, &testing->images[i], K, fptr) == testing->labels[i]) {
            correct++;
        }
    }
    if (write(p_out, &correct, sizeof(correct)) < 0) {
        perror("write");
        exit(1);
    };
    return;
...
double distance_cosine(Image *a, Image *b){
    ...
    double sum_denumerator1 = 0;
    double sum_denumerator2 = 0;
    for (int i = 0; i < a->sx * a->sy; i++) {
        sum_numerator += (a->data[i] * b->data[i]);
        sum_denumerator1 += (a->data[i] * a->data[i]);
        sum_denumerator2 += (b->data[i] * b->data[i]);
    }
    return 2 * acos(sum_numerator / (sqrt(sum_denumerator1) * sqrt(sum_denumerator2))) / M_PI;
}
